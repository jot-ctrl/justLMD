...
# Wayland/ヘッドレス環境での Open3D 初期化失敗を避ける
os.environ.setdefault('OPEN3D_CPU_RENDERING', '1')

import nosmpl.smpl_onnx as smpl_onnx_module
try:
    import onnxruntime as _onnxruntime
except ImportError as exc:
    raise ImportError('onnxruntime is required for SMPL inference. Install via pip install onnxruntime or onnxruntime-gpu.') from exc
if not hasattr(smpl_onnx_module, 'os'):
    smpl_onnx_module.os = os

if not hasattr(smpl_onnx_module, 'rt'):
    smpl_onnx_module.rt = _onnxruntime

if not hasattr(smpl_onnx_module, 'download'):
    def _nosmpl_download(url, save_path, overwrite=False):
        if os.path.exists(save_path) and not overwrite:
            return
        os.makedirs(os.path.dirname(save_path) or '.', exist_ok=True)
        print('Downloading %s -> %s' % (url, save_path))
        with urllib.request.urlopen(url) as response, open(save_path, 'wb') as out_file:
            shutil.copyfileobj(response, out_file)
    smpl_onnx_module.download = _nosmpl_download
...
class LMD_Dataset(Dataset): 
    def __init__ (self, data_dir, songs_collection, name='LMD'):
        ...
        self.can_visualize = os.environ.get('JUSTLMD_ENABLE_OPEN3D') in ('1', 'true', 'TRUE')
        if not self.can_visualize:
            print('[LMD_Dataset] JUSTLMD_ENABLE_OPEN3D が有効化されていないため、Open3D 可視化をスキップします。')
    ...
    def __getitem__(self,index):
        key = self.indexing.get(str(index))
        if key is None:
            key = self.indexing.get(index)
        if key is None:
            raise KeyError(f'Sample index {index} not found in dataset indexing')
    ...
    def visualize(self,seq_name, save_img_folder=None, inf=False, glob_trans=True):
        if not self.can_visualize:
            print('Open3D 環境が無いため visualize をスキップ: %s' % seq_name)
            return
    ...
    def export(self, seq_name, save_dir=None, inf=False, glob_trans=True):
        if not self.can_visualize:
            print('Open3D が利用できないため export をスキップ: %s' % seq_name)
            return
